import $ from '../lib/jquery-3.4.1'; //Параметри: текст заголовку і пунктів меню
...
export class Header {
  ...
  _init() {
    ...
    for (let key in this._options) {
      ...
      let span = $("<span></span>").addClass(classStr).text(`${key}: ${this._options[key]}`); //Додає обгортку для показу блоку біля поля Score
...
export class Info {
  ...
  animate(dt, duration, text) {
    ...
    let textNoSpace = text.replace(/\s+/g, ""); //"textNoSpace.length + 1", бо перед показом перщшої літери проходить 1 period
