...
import { GameOverScene } from "./GameOverScene"; //Основна сцена гри, в якій відбуваються всі події
...
export class GameScene {
  constructor(game) {
    ...
    this._deffBoard = $.Deferred(); //this._isLoadBall = false;
  ...
  _initBall() {
    ...
    this._deffBall.done(() => {
      this._ball.setRadius(this._ballElem.outerWidth() / 2); //this._isLoadBall = true;
  ...
  _calcBallPosition(dt, ball) {
    ...
    let distance = 0; //шукаємо об'єкт, з яким першим відбулося зіткнення
  ...
  _calcTouchedBlockPos(ball) {
    ...
    let normalV = ball.getNormal(new Vector(0, Math.sign(+ball.direction.y))); //якщо є дотик до сторони, то не перевіряти далі
  ...
  _calcVertexRebound(block, ball, vertex) {
    ...
    let normal = new Vector(-vecCentrToVertex.y, vecCentrToVertex.x); //кут між напрямком руху і вектором з центра до вершини
